...
class BigNumberVis extends React.PureComponent<BigNumberVisProps> {
  ...
  renderHeader(maxHeight: number) {
    const { bigNumbers, headerFormatter, width } = this.props;
    const text =
      bigNumbers[0] === null ? t('No data') : headerFormatter(bigNumbers[0]);
  ...
  renderSubheader(maxHeight: number) {
    ...
    let text = subheader;
    if (this.props.bigNumbers[0] === null) {
  ...
  render() {
    ...
    return (
      <div className={className} style={{ height }}>
        ...
        <Row align="middle">
          {this.props.bigNumbers.map((bigNumber: number, index: number) => (
            <Col
              span={24 / this.props.colsPerRow}
              className={this.props.metricNames[index]}
              style={{
                marginTop: this.props.headingSize / 2,
                marginBottom: this.props.headingSize / 2,
                textAlign: 'center',
              }}
            >
              <div
              ...
              >
                {this.props.metricNames[index]}
              </div>
              <div
                style={{
                  fontSize: this.props.bignumberSize,
                  fontWeight: 'bold',
                  fontFamily: this.props.fontFamily,
                }}
              >
                {this.props.headerFormatter(bigNumber)}
              </div>
            </Col>
          ))}
        </Row>
